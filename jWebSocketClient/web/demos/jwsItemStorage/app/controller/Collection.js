Ext.define('IS.controller.Collection', {
	...
	__callSubscription: function (aCollectionName, aItemDefinition) {
		...
		var lView = Ext.create('IS.view.collection.EnterAccessPwd', {
			...
			doAction: function () {
				...
				Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'subscribe', lArguments, {
					success: function () {
	...
	open: function (aCollectionName) {
		...
		if (!this.getWorkspace().setActiveTab(aCollectionName)) {
			Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'findCollection', {
				collectionName: aCollectionName
			}, {
				success: function (aResponse) {
					var lSubscribed = aResponse.data.subscribed;
					Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'findDefinition', {
	...
	init: function () {
		...
		Ext.jwsClient.on('beforesend', function (aToken) {
				...
				click: function (aButton) {
					var lView = Ext.create('IS.view.collection.EnterSecretPwd', {
						...
						doAction: function () {
							...
							Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'removeCollection', lArguments, {
								success: function () {
				...
				click: function (aButton) {
					var lView = Ext.create('IS.view.collection.EnterSecretPwd', {
						...
						doAction: function () {
							...
							Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'restartCollection', lArguments, {
								success: function () {
									lView.close();
								}
							});
				...
				click: function (aButton) {
					var lView = Ext.create('IS.view.collection.EnterSecretPwd', {
						...
						doAction: function () {
							...
							Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'clearCollection', lArguments, {
								success: function () {
									lView.close();
								}
							});
			...
			'c_edit button[action=edit]': {
				click: function (aButton) {
					...
					if (lForm.getForm().isValid()) {
						...
						Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'editCollection', lArguments, {
							success: function () {
								lWindow.close();
							}
						});
