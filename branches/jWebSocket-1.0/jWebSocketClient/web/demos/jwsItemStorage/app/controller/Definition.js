		...
		this.control({
			...
			'd_create button[action=create]': {
				click: function (aButton) {
					...
					for (var lKey in lValues) {
						if (typeof lValues[lKey] === "undefined") {
							delete lValues[lKey];
						}
					}
					...
					var lRegisterDefFn = function () {
						Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'registerDefinition', lValues, {
							success: function () {
					...
					if ('create' == lView.mode) {
						...
						Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'removeDefinition', {
			...
			'd_editattr button[action=save]': {
				click: function (aButton) {
					...
					for (var lKey in lValues) {
						if ("undefined" === typeof lValues[lKey]) {
							delete lValues[lKey];
						}
					}
				...
				click: function () {
					...
					Ext.Msg.show({
						...
						fn: function (aButton) {
							...
							Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'removeDefinition', lArguments, {
								success: function () {
									self.getDefinitionsStore().reload();
								}
							});
