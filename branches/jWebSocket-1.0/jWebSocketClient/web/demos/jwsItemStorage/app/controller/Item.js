			...
			success: function (aResponse) {
				...
				var lView = Ext.create('IS.view.collection.EnterSecretPwd', {
					...
					doAction: function () {
						...
						Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'authorize', lArguments, {
							success: function () {
								aAction();
								lView.close();
							}
		...
		this.control({
			...
			'i_create button[action=save]': {
				click: function (aButton) {
					...
					Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'saveItem', {
					...
					self.checkAuthorization(lCollectionName, function () {
						Ext.Msg.show({
							...
							fn: function (aButton) {
								...
								Ext.jwsClient.send(jws.ItemStoragePlugIn.NS, 'removeItem', lArguments);
							}
						});
					});
				}
