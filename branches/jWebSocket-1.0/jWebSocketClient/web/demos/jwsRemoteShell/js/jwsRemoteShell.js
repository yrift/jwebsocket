...
$.widget("jws.RemoteShell", {
	...
	registerEvents: function () {
		...
		w.rShell.addOnMessageListeners();
	...
	history: function (aEl) {
		if (aEl && typeof aEl.val === "function") {
			var lOldContent = aEl.val().split("\n");
			var lTemp = "";
			for (var i in lOldContent) {
				if (i < lOldContent.length - 1) {
					lTemp += lOldContent[i] + "\n";
				}
			}
			aEl.val(lTemp + w.rShell.mHistory[w.rShell.mHistoryCount]);
		}
	},
	 ...
	 *   });
	 **/
	addOnMessageListeners: function () {
