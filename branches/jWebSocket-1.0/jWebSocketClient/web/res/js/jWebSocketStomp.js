...
(function () {
    ...
    STOMPWebSocket = function (aUrl, aSubprotocol, aUsername, aPassword) {
        var self = this;
        var mUsername = aUsername || "guest";
        var mPassword = aPassword || "guest";
        ...
        self.sendChannel = "/queue/" + lQuery["cluster"];
        self.receiveChannel = "/topic/" + lQuery["cluster"];
            ...
            try {
                ...
                if (typeof (lMessage) === "object" && lMessage[ "jwsWrappedMsg" ]) {
                    if ("info" === lMessage.type && "ack" === lMessage.name) {
                        self.stomp.send(self.sendChannel, {
                            msgType: "ACK",
